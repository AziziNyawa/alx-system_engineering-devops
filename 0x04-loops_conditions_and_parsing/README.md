...
Project done during **Full Stack Software Engineering studies** at **Alx Software engineering**. It aims to learn about loops (`while`, `until` and `for`), condition statements (`if`, `else`, `elif` and `case`), shebangs and how to create SSH keys with **Bash**.
...
| -------- | ----------- |
| `0-RSA_public_key.pub` | File that stores a RSA public key to access servers via SSH and also creating it|
| `1-for_best_school` | Displays `Alx Software engineering studies` 10 times with a `for` loop |
